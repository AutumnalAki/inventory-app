...
function getPortalSlug(): string {
  ...
  return toSlug(expectedName || expectedToken || "incident");
}

export default async function IncidentPortalQrPage() {
  ...
  const portalUrl = `${protocol}://${host}/incident-portal/${encodeURIComponent(slug)}`;
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white p-6 sm:p-8">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Incident Report QR</h1>
        <p className="text-gray-400 mt-2 text-sm sm:text-base">
          Scan this code to open the public student incident report form.
        </p>

        <div className="mt-6 bg-white/5 border border-white/10 rounded-2xl p-5 sm:p-6">
          <div className="bg-white rounded-xl p-4 inline-block">
            <img src={qrDataUrl} alt="Incident report QR code" className="w-64 h-64 sm:w-80 sm:h-80" />
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <a
              href={qrDataUrl}
              download={`incident-report-${slug}-qr.png`}
