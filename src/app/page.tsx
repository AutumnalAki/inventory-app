    ...
    <div className="relative min-h-screen text-white overflow-hidden bg-[#050505]">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-black/20 backdrop-blur-lg">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <button onClick={() => setShowAboutFooter(!showAboutFooter)} className="text-sm font-medium text-gray-400 hover:text-white transition-colors">About</button>
            <Link href="/signin">
              <button className="bg-white text-black px-5 py-2 rounded-full text-sm font-bold hover:bg-gray-200 transition-all">Sign In</button>
            </Link>
          </div>
      ...
      <main className="relative pt-32 pb-20 px-6">
        <div className="max-w-7xl mx-auto flex flex-col items-center text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full mb-8">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-gray-400 text-lg md:text-xl max-w-2xl mb-10 leading-relaxed">
          ...
          <motion.div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto mt-10">
            ...
            <Link href="/signin">
              <button className="group bg-white/5 hover:bg-white/10 border border-white/10 text-white px-8 py-4 rounded-2xl font-bold transition-all w-full sm:w-auto flex items-center justify-center gap-2">
                <Image src="/favicon.ico" alt="Sign In Icon" width={18} height={18} className="rounded-sm opacity-80 group-hover:opacity-100 transition-opacity" />
                Sign In
              </button>
            </Link>
          </motion.div>
      ...
      <AnimatePresence>
        {showAboutFooter && (
          <motion.footer initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="fixed bottom-0 left-0 w-full z-40 bg-black/80 backdrop-blur-md border-t border-white/10 p-6">
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-2">
              ...
              <button onClick={() => setShowAboutFooter(false)} className="text-xs text-gray-500 hover:text-white transition-colors">Close [x]</button>
            </div>
          </motion.footer>
        )}
      </AnimatePresence>
