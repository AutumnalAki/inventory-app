...
// === LOGO ===
const LOGO_URL = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ccircle cx='50' cy='50' r='48' fill='%231e3a5f' stroke='%23003d82' stroke-width='2'/%3E%3Ctext x='50' y='55' font-size='32' font-weight='bold' fill='white' text-anchor='middle' dominant-baseline='middle' font-family='Arial'%3ECDM%3C/text%3E%3C/svg%3E";
...
const DetailInput: React.FC<{
  ...
  <div className="flex items-stretch h-full min-h-[2.25rem] border-b-2 border-black last:border-b-0 bg-white hover:bg-orange-50/30 transition-colors">
    ...
    <input
      ...
      className="flex-1 px-3 py-1 bg-transparent outline-none text-[10px] sm:text-xs font-medium placeholder:text-stone-300 placeholder:italic focus:bg-orange-50/50"
    />
  </div>
);

// === HELPER: SignatureBlock ===
const SignatureBlock: React.FC<{ label: string; title: string; name?: string }> = ({ label, title, name }) => (
  <div className="border-r-2 border-black last:border-r-0 p-2 text-center border-t-2 flex flex-col">
    ...
    <p className="text-[8px] sm:text-[9px] font-bold uppercase tracking-tight">{title}</p>
    <p className="text-[7px] sm:text-[8px] font-normal">{label}</p>
    {name && <p className="text-[7px] sm:text-[8px] font-bold">{name}</p>}
  </div>
);
...
export default function RequisitionFormTestingPage() {
  ...
  const [form, setForm] = useState<Partial<RequisitionForm>>({
    ...
    room: "",
    timeOfUse: "",
    items: Array(10).fill({ name: "", quantity: 0, unit: "", dateOut: "", dateIn: "" }),
  });
  ...
  return (
    <div className="min-h-screen bg-stone-100 py-4 px-3 sm:px-6">
      <style>{`@media print { body { background: white; } .no-print { display: none !important; } .bg-stone-100 { background: white !important; } }`}</style>

      {/* Toolbar */}
      <div className="max-w-5xl mx-auto mb-4 flex gap-2 flex-wrap no-print">
      ...
      {/* Form Paper */}
      <div ref={formRef} className="max-w-5xl mx-auto bg-white shadow-2xl">
        <form onSubmit={handleSubmit} className="border-4 border-black">
          {/* Header */}
          <div className="border-b-4 border-black p-4 sm:p-6 flex justify-between gap-4 items-start">
            <div className="flex gap-4 flex-1">
              <div className="w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0 bg-blue-100 border-4 border-blue-900 rounded-full flex items-center justify-center">
                <img src={LOGO_URL} alt="CDM Logo" className="w-16 sm:w-20 h-16 sm:h-20" />
              </div>
              <div className="flex flex-col justify-center">
                <h1 className="text-base sm:text-xl font-bold uppercase tracking-tight">Colegio de Muntinlupa</h1>
                <h2 className="text-lg sm:text-2xl font-black uppercase">Requisition Form</h2>
                <p className="text-[10px] sm:text-xs font-bold uppercase text-stone-700">Equipment, Supplies and Apparatus</p>
              </div>
            </div>
            ...
            <div className="border-2 border-black text-[10px] w-56 flex-shrink-0">
              ...
              <div className="grid grid-cols-3 text-[9px] font-bold">
                <div className="border-r-2 border-b-2 border-black p-1 text-center"><span className="text-[8px]">Effective Date</span></div>
                <div className="border-r-2 border-b-2 border-black p-1 text-center"><span className="text-[8px]">Revision No.</span><div>00</div></div>
                <div className="border-b-2 border-black p-1 text-center"><span className="text-[8px]">Revision Date</span></div>
              </div>
              <div className="border-t-2 border-black p-1 text-center bg-yellow-50 text-[10px] font-mono font-bold">AUTOGEN-DLI-SUBMIT</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 border-b-4 border-black">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-stretch h-full min-h-[2.25rem] border-b-2 border-black bg-white hover:bg-orange-50/30">
                <div className="w-32 shrink-0 flex items-center px-3 font-bold border-r-2 border-black">
                  <span className="text-[9px] sm:text-xs">Time of use:</span>
                </div>
                <input type="text" value={form.timeOfUse || ""} onChange={(e) => handleFieldChange("timeOfUse", e.target.value)} placeholder="Start to End" className="flex-1 px-3 py-1 bg-transparent outline-none text-[10px] sm:text-xs font-medium placeholder:italic focus:bg-orange-50/50" />
              </div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto w-full border-b-4 border-black">
            <table className="w-full border-collapse min-w-[600px]">
              <thead>
                <tr className="text-center text-[9px] sm:text-xs font-bold bg-stone-200 border-b-2 border-black">
                  <th className="border-r-2 border-black py-2 px-1 w-[45%]">Equipment/Supplies/Apparatus</th>
                  <th className="border-r-2 border-black py-2 px-1 w-[12%]">Quantity</th>
                  <th className="border-r-2 border-black py-2 px-1 w-[10%]">Unit</th>
                  <th className="border-r-2 border-black py-2 px-1 w-[16%]">Date and Time Out</th>
                  <th className="py-2 px-1 w-[17%]">Date and Time In</th>
                </tr>
              </thead>
              <tbody>
                {(form.items || []).map((item, idx) => (
                  <tr key={idx} className="text-[9px] sm:text-xs border-b-2 border-black hover:bg-orange-50/20">
                    <td className="border-r-2 border-black p-1 relative">
                      <input type="text" value={item.name} onChange={(e) => handleItemChange(idx, "name", e.target.value)} onFocus={() => item.name && showSuggestions(idx, item.name)} placeholder={idx === 0 ? "Search items..." : ""} className="w-full bg-transparent outline-none text-[10px] sm:text-xs font-medium focus:bg-orange-50 px-1" />
                      <AnimatePresence>
                    ...
                    <td className="border-r-2 border-black p-1 text-center">
                      <input type="number" min="0" value={item.quantity || 0} onChange={(e) => handleItemChange(idx, "quantity", e.target.value)} className="w-full bg-transparent outline-none text-center text-[10px] sm:text-xs focus:bg-orange-50" />
                    </td>
                    <td className="border-r-2 border-black p-1 text-center">
                      <select value={item.unit || "pcs"} onChange={(e) => handleItemChange(idx, "unit", e.target.value)} className="w-full bg-transparent outline-none text-[10px] sm:text-xs focus:bg-orange-50">
                    ...
                    <td className="border-r-2 border-black p-1 text-center">
                      <input type="text" value={item.dateOut || ""} onChange={(e) => handleItemChange(idx, "dateOut", e.target.value)} placeholder="MM/DD/YY" className="w-full bg-transparent outline-none text-center text-[10px] sm:text-xs focus:bg-orange-50" />
                    </td>
                    <td className="p-1 text-center">
                      <input type="text" value={item.dateIn || ""} onChange={(e) => handleItemChange(idx, "dateIn", e.target.value)} placeholder="MM/DD/YY" className="w-full bg-transparent outline-none text-center text-[10px] sm:text-xs focus:bg-orange-50" />
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="grid grid-cols-4 border-b-4 border-black">
            <SignatureBlock label="Requested by:" title="Student/Instructor" />
            <SignatureBlock label="Endorsed by:" title="Instructor/Adviser" />
            <SignatureBlock label="Released by:" title="Lab. Technician" />
            <SignatureBlock label="Approved by:" title="Lab. Head/Prof. Chair" name="ENGR. JOHN DANIEL SA..." />
          </div>

          {/* Date Row */}
          <div className="grid grid-cols-4 border-b-4 border-black">
            <div className="border-r-2 border-black p-2 text-center text-[8px] font-bold">Date: _________</div>
            <div className="border-r-2 border-black p-2 text-center text-[8px] font-bold">Date: _________</div>
            <div className="border-r-2 border-black p-2 text-center text-[8px] font-bold">Date: _________</div>
            <div className="p-2 text-center text-[8px] font-bold">Date: _________</div>
          </div>
          ...
          <div className="bg-yellow-50 p-4 flex gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-700 mt-1">By submitting this form, I acknowledge responsibility for the equipment requested. I agree to adhere to laboratory safety protocols and return all items to their original condition.</p>
            </div>
          </div>
        </form>
      </div>
      ...
      <AnimatePresence>
        {showSuccessModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 no-print">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-6 max-w-md w-full text-center">
              <CheckIcon size={48} className="text-emerald-400 mx-auto mb-4" />
              <h3 className="font-bold text-black text-lg mb-2">Requisition Submitted!</h3>
              <p className="text-gray-700 text-sm mb-4">Your form has been saved and is ready for processing.</p>
              <button onClick={() => { setShowSuccessModal(false); router.push("/dashboard"); }} className="w-full bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-700 font-bold py-2 rounded-lg">
                Return to Dashboard
              </button>
            </motion.div>
          </motion.div>
