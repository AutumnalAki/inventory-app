...
import { 
  ...
  MapPin, Hash, FileText, MoreHorizontal, X, Save, Search
} from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useInventory, Item } from "@/context/InventoryContext";

// --- 1. MAPPING CONFIGURATION ---
// Key = What shows on the Button (Short)
// Value = What is stored in the Database (Long)
const LAB_MAPPING: Record<string, string> = {
  ...
  "ECE Lab": "ECE Laboratory" // Included in case you add this later
};
...
export default function InventoryPage() {
  ...
  const [selectedLab, setSelectedLab] = useState("All Labs"); // Stores the Short Name (Button Text)
  ...
  // --- LOGIC: FILTER -> SEARCH -> SORT -> PAGINATE ---
  const processedData = useMemo(() => {
    let data = [...inventory];

    // 1. Filter by Location (Using the Mapping)
    ...
    return data;
  }, [inventory, selectedLab, filterStatus, sortOption, searchTerm]);
  ...
  const handleSaveItem = (e: React.FormEvent) => {
    ...
    if (isEditing && currentId !== null) {
      updateItem(currentId, itemToSave as any);
    } else {
      addItem(itemToSave as any);
    }
        ...
        <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto shrink-0">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-2 text-xs text-white focus:outline-none focus:border-indigo-500 transition-colors placeholder:text-gray-500"
            />
          </div>
        </div>

        {/* Center: Tabs (Now Functional) */}
        <div className="flex-1 overflow-x-auto no-scrollbar mask-linear-fade flex justify-center w-full">
            <div className="flex items-center gap-1 min-w-max px-2">
                {LAB_TABS.map((labShortName) => (
                <button 
                    key={labShortName} 
                    onClick={() => handleFilterChange(setSelectedLab, labShortName)} 
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap border ${selectedLab === labShortName ? "bg-white text-black border-white shadow-sm" : "text-gray-400 border-transparent hover:text-white hover:bg-white/5"}`}
                >
                    {labShortName}
                </button>
                ))}
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm shadow-xl flex-1 flex flex-col">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5 text-sm">
              {currentItems.length > 0 ? (
                  ...
                  <tr key={item.id} className="group hover:bg-white/[0.07] transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2 text-gray-300">
                          <div className="bg-indigo-500/10 p-1.5 rounded-md text-indigo-400"><MapPin size={12} /></div>
                ...
                <tr><td colSpan={8} className="p-8 text-center text-gray-500 text-sm">No items found matching your filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="mt-auto p-4 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-gray-500">
          ...
          <div className="flex gap-2">
            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg font-medium hover:bg-white/20 hover:text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed">Previous</button>
            <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages || totalPages === 0} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg font-medium hover:bg-white/20 hover:text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed">Next</button>
          </div>
        </div>
      </div>
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="relative bg-[#111] border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden">
              ...
              <form onSubmit={handleSaveItem} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5"><label className="text-xs font-medium text-gray-400 uppercase">Item Name</label><input required type="text" placeholder="e.g. V5 Smart Motor" value={newItem.name} onChange={(e) => setNewItem({...newItem, name: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors" /></div>
                  <div className="space-y-1.5"><label className="text-xs font-medium text-gray-400 uppercase">Control ID</label><input required type="text" placeholder="e.g. VEX-099" value={newItem.controlId} onChange={(e) => setNewItem({...newItem, controlId: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors" /></div>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-1.5"><label className="text-xs font-medium text-gray-400 uppercase">Quantity</label><input required type="number" min="0" value={newItem.quantity} onChange={(e) => setNewItem({...newItem, quantity: parseInt(e.target.value)})} className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors" /></div>
                   <div className="space-y-1.5 col-span-2"><label className="text-xs font-medium text-gray-400 uppercase">Location</label><div className="relative">
                     
                     {/* FIX: Map buttons correctly to Database Values */}
                     <select required value={newItem.location} onChange={(e) => setNewItem({...newItem, location: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors appearance-none">
                       ...
                       {LAB_TABS.filter(lab => lab !== "All Labs").map((labShortName) => (
                         <option key={labShortName} value={LAB_MAPPING[labShortName]} className="bg-gray-900">
                           {labShortName}
                         </option>
                       ))}
                     </select>
                     
                     <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" size={14} /></div></div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5"><label className="text-xs font-medium text-gray-400 uppercase">Supplier</label><input type="text" placeholder="e.g. VEX Robotics" value={newItem.supplier} onChange={(e) => setNewItem({...newItem, supplier: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors" /></div>
                  <div className="space-y-1.5"><label className="text-xs font-medium text-gray-400 uppercase">Condition</label><div className="relative"><select value={newItem.condition} onChange={(e) => setNewItem({...newItem, condition: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors appearance-none"><option className="bg-gray-900">Available</option><option className="bg-gray-900">Broken</option><option className="bg-gray-900">For Repairs</option></select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" size={14} /></div></div>
                </div>
                 <div className="space-y-1.5"><label className="text-xs font-medium text-gray-400 uppercase">Remarks</label><textarea rows={2} placeholder="Optional notes..." value={newItem.remarks} onChange={(e) => setNewItem({...newItem, remarks: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors resize-none"/></div>
                <div className="flex justify-end gap-3 pt-4 border-t border-white/10"><button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors">Cancel</button><button type="submit" className="flex items-center gap-2 px-6 py-2 text-sm font-bold bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-all shadow-lg shadow-indigo-900/20"><Save size={16} />{isEditing ? "Update Item" : "Save Item"}</button></div>
              </form>
            </motion.div>
          </div>
        )}
...
function StockBadge({ status }: { status: string }) {
  const styles: Record<string, string> = { "In Stock": "bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-[0_0_10px_rgba(16,185,129,0.1)]", "Low Stock": "bg-amber-500/10 text-amber-400 border-amber-500/20 shadow-[0_0_10px_rgba(245,158,11,0.1)]", "Out of Stock": "bg-rose-500/10 text-rose-400 border-rose-500/20 shadow-[0_0_10px_rgba(244,63,94,0.1)]" };
